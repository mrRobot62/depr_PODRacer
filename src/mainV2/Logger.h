...
class SLog {
  public:
    ...
    void once_info(uint8_t *address_value, const char *text, const char *tname="?", bool cr=true) {
      if (*address_value == 0) {
        info(text, tname, cr);
      }
      *address_value = 1;
    ...
    void once_warn(uint8_t *address_value, const char *text, const char *tname="?", bool cr=true) {
      if (*address_value == 0) {
        warn(text, tname, cr);
      }
      *address_value = 1;
    ...
    void once_error(uint8_t *address_value, const char *text, const char *tname="?", bool cr=true) {
      if (*address_value == 0) {
        error(text, tname, cr);
      }
      *address_value = 1;
    ...
    void data(TaskData *data, const char *tname="?", const char *tgroup="-", bool printCH=true, bool printFData=false, bool printLData = false, bool pidData = false, bool mode=0) {
      ...
      if (_visualizer_mode == 1 or mode == 1) {
        ...
        if (printCH) {
          sprintf(tmp, 
            //"UPD:%d,ARM:%d,FAIL:%d,LF:%d,CH[R]:%04d CH[P]:%04d,CH[T]:%04d,CH[H]:%04d,CH[ARM]:%04d,CH[AUX1]:%04d,CH[AUX2]:%04d,CH[AUX3]:%04d",
            "UPD:%d,ARM:%d,FAIL:%d,LF:%d,CHANNELS(R,P,Y,T,H,ARM,AUX1-3):%04d,%04d,%04d,%04d,%04d,%04d,%04d,%04d,%04d",
            data->data.updated,
            data->data.is_armed,
            data->data.failsafe,
