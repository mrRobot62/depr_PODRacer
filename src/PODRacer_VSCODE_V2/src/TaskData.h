...
typedef struct {
  ...
  long      start_millis;         // store begin of update method (to measure runtime of this method)
  long      end_millis;           // store end of update methode (now we know, how lng this method run )
  ...
  float     fdata[DATA_SIZE];     // can be used to store number of float values
  float      pid_rpyth[5];         // can be used to store number of float values
